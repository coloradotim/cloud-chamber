...
describe("App", () => {
  ...
  it("renders cloud-water context in unified Explore", async () => {
    ...
    const axisKey = within(viewer).getByLabelText("Domain axis dimensions");
    expect(within(axisKey).getByText("x")).toBeInTheDocument();
    expect(within(axisKey).getByText("y")).toBeInTheDocument();
    expect(within(axisKey).getByText("z")).toBeInTheDocument();
    expect(within(axisKey).getAllByText("-3.2 km to 3.2 km")).toHaveLength(2);
    expect(within(axisKey).getByText("0 km to 3 km")).toBeInTheDocument();
