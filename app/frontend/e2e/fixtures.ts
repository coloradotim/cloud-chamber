...
export const results = [
  {
    ...
    controls: { low_level_humidity: "baseline", surface_heating: "baseline", cap_strength: "baseline" },
    status: "ingested",
  ...
  },
  {
    ...
    run_size_preset: "quick_look",
    physical_question: "How does insufficient low-level moisture prevent shallow cumulus formation?",
...
export async function mockCloudChamberApis(page: Page) {
  ...
  await page.route("**/api/dry-run-package", (route) =>
    json(route, {
      ...
      generated_files: ["run_manifest.json", "case_manifest.json", "namelist.input", "input_sounding"],
      report: {
  ...
  await page.route("**/api/results/ingest", (route) =>
    json(route, { result_id: "result-baseline", run_id: "dry-run-baseline", diagnostics_summary: "cloud formed; rain detected" }),
  );
  ...
  await page.route("**/api/results/*/visualization/point-cloud**", (route) =>
    json(route, {
      ...
      scenario_id: "baseline-shallow-cumulus",
      field: fieldCatalog("result-baseline").available_fields[0],
      selection: { field: "qc", time_index: 1, time_seconds: 1800, threshold: 0.000001, max_points: 50000 },
