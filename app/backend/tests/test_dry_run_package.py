...
def test_deep_tower_low_level_lift_package_uses_stock_iinit9_path(
    tmp_path: Path,
) -> None:
    observed = parse_igra_station_text(
        IGRA_FIXTURE,
    ...
    result = generate_dry_run_package(
        scenario_data=load_baseline_template(),
        runtime_home=tmp_path,
        run_id="run-deep-tower-low-level-lift",
    ...
    assert manifest.run_recipe == "deep_tower_low_level_lift"
    assert manifest.run_recipe_display_name == "Deep-Tower Low-Level Lift Benchmark"
    assert manifest.recipe_id == "deep_tower_low_level_lift_v0"
    assert manifest.recipe_display_name == "Deep-Tower Low-Level Lift Benchmark v0"
    assert manifest.assumption_set_id == "deep_tower_low_level_lift_v0_assumptions"
    assert manifest.assumption_mode == "explicit_low_level_lift"
    assert manifest.trigger_type == "forced_convergence"
    assert manifest.trigger_parameters == {
        "cm1_iinit": 9,
        "cm1_trigger": (
    ...
    assert manifest.run_configuration["domain_size"] == "deep_tower_120km"
    assert manifest.run_configuration["surface_flux_mode"] == "disabled"
    assert manifest.run_configuration["initiation_method"] == "cm1_iinit_9_forced_convergence"
    assert manifest.run_configuration["cm1_values"]["runtime_seconds"] == 7200
    assert manifest.run_configuration["cm1_values"]["rayleigh_damping_start_m"] == 15000
    assert manifest.recipe_assumptions["trigger"]["mode"] == "cm1_iinit_9_forced_convergence"
    assert manifest.recipe_assumptions["observed_sounding"]["wind_profile"] == (
        "required_complete_rendered_u_v_profile"
    )
    assert manifest.recipe_assumptions["surface_fluxes"]["mode"] == "disabled"
    ...
    assert (
        ...
        "Deep-Tower low-level-lift benchmark trigger."
        in manifest.pre_run_validation_report["hypothesis_recipe_alignment"]["reasons"]
    )
    assert (
        "explicit_low_level_lift_supplied_not_a_real_observed_trigger"
        in manifest.pre_run_validation_report["caveats"]
    )
    assert report["recipe_id"] == "deep_tower_low_level_lift_v0"
    assert report["trigger_type"] == "forced_convergence"
    assert report["variant_metadata"]["surface_flux_mode"] == "disabled"
    assert (
        "stock CM1 iinit=9 forced low-level convergence" in (report["variant_metadata"]["mapping"])
    )
    assert "explicit stock-CM1 iinit=9" in report["cm1_mapping_status"]
    assert "did not produce cloud" in report["cm1_mapping_status"]
    assert case_manifest["contract"]["recipe_id"] == "deep_tower_low_level_lift_v0"
    assert case_manifest["contract"]["trigger_type"] == "forced_convergence"
    assert "testcase  =  0," in namelist
    assert "isnd      =  7," in namelist
