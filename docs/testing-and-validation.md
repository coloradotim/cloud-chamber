...
CM1 runtime floating-point exception flags such as `IEEE_INVALID_FLAG`, `IEEE_DIVIDE_BY_ZERO`, and `IEEE_OVERFLOW_FLAG` should be preserved as runtime evidence and evaluated by runtime-integrity checks. Automated diagnostics should then check whether target fields contain non-finite values and whether `cm1out_stats.nc` shows sentinel collapse. If runtime integrity fails, science outcomes must remain visible as untrusted rather than being summarized as clean results. If root-cause investigation requires CM1 source-level debugging, that belongs in a separate issue rather than CI.
