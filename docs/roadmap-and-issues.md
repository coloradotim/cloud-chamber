...
- #73 builds Explore / 2-D Slices on top of the #72 fields/slice API before the full 3-D viewer so field orientation, time indexing, vertical coordinates, scaling, and basic cloud evolution can be checked. It opens from selected result context, enables field/time selection, shows horizontal and vertical slices, and keeps the browser away from raw NetCDF parsing.
- #77 builds Explore / 3-D View from the selected result context: scene container, projection/view controls, zoom, reset view, time slider shell, field selector shell, loading/empty/error states, and provenance/rendering labels. It does not render cloud water, slices, or raw NetCDF data in the browser.
...
- #105 makes Explore physically interpretable rather than debug-like: the backend provides native-grid default view locations for `qc` and `w`, Explore / 2-D Slices defaults to one primary heatmap with `Horizontal` / `Vertical X` / `Vertical Y` / `Compare` modes, Explore / 3-D View adds domain box/axes/height/floor/time/threshold context, slice planes are toggleable and secondary, and view presets expose Cloud overview, Vertical cross-section, Top-down slice, and Updraft view.
