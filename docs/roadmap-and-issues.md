...
notebook edit state, and open/compare actions belong in the first read, while
...
- name/tag/notes/search/filter/delete.
- result-card / experiment-notebook model.
- open ingested result in visualizer.
- duplicate/tweak/rerun foundation later, after replay/inspect/notebook edits are reliable.

Implementation anchor:

- #30 should make replayable/inspectable result cards with editable notebook fields the core behavior. Duplicate/tweak/rerun should remain optional or later.
...
- #70 adds the backend Result Card / Experiment Notebook API over ingested metadata: list/get cards, update name/tags/notes, and summarize run ID, scenario, run-size preset, physical question, diagnostics, caveats, and output files without rerunning CM1.
- #71 adds the first frontend Results Library shell over the #70 API: result cards, selected result detail/notebook card, diagnostics/caveats/output summaries, and editable name/tags/notes. It must not implement replay or 3-D visualization.
...
- #109 adds runtime storage management under Results / Storage over the runtime storage backend: total runtime-home usage, 50 GB warning-threshold state, largest run directories, result-card names when associated, output summaries, dry-run delete preview, explicit confirmed deletion, and disabled cleanup for running runs.
- #189 is the next lifecycle-cleanup gate after #173/#190: Storage should read as runtime inventory for the same package/run/result lifecycle that Build launches and Results reviews. It joins result-card names to run directories, shows ready-to-run/running/completed/ready-to-ingest/ingested/legacy-saved/malformed states clearly, exposes safe non-destructive actions, and keeps deletion behind Storage preview/confirmation without forcing users to manage local folders by raw run ID.
- #203 audits the current package/run/result/storage lifecycle contract in `docs/ingest-results-storage-lifecycle.md`. It confirms the desired product model is only partially compatible today: ingest already creates Results entries, and title/tags/notes edits are local card state, but `result_metadata.json` and `result_card.json` still live inside deletable run directories and frontend/backend cleanup protection use different sources of truth.
...
- #214 defines the output product specification before further visualization or diagnostics expansion. It separates raw CM1 output, runtime manifests/logs, result metadata/notebook state, output product manifests, derived scientific products, visualization-ready payloads, future render-ready products, and external export bundles. It also preserves the #215 performance lesson: field catalogs/defaults should come from metadata or bounded products, and selected timestep requests should resolve through explicit file/time mapping rather than full-sequence NetCDF opens.
...
- M5 and broader renderer/scenario expansion should start only after the local product loop is useful and stable: Build can package/launch/ingest, Results can review/edit/compare, Storage can manage runtime files safely, unified Explore can explain selected results/regions, and tests cover the current loop with mocked local state.
