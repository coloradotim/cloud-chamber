...
- #173 reframes Build as a guided experiment setup plus local run launchpad rather than a one-package-at-a-time wizard. It should show local packages/runs/results across pipeline states, move eligible packages forward through existing backend APIs, and route cleanup to Results / Storage.
...
- #189 is the follow-up to clarify Storage's product role and integration with Build and Results: run/package pipeline visibility, result-card identity joins, cleanup readiness, and safe actions without forcing users to manage local folders by raw run ID.
