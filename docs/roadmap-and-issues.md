...
- #71 adds the first frontend Results Library shell over the #70 API: a scan-friendly results table, selected result detail/notebook card, diagnostics/caveats/output summaries, saved/protected state, and editable name/tags/notes. It must not implement field inspection, replay, or 3-D visualization; an Inspect fields affordance should remain disabled until #73.
